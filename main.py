...
def fifa2020_players():
    base_page = 'https://www.futwiz.com/en/fifa20/players'
    base_page_href = 'https://www.futwiz.com'
    session = requests.Session()
    max_page = 955
    min_page = 0
    ...
    df.to_csv('players_2020.csv', index=False)

def fifa2020_history_thread(core_id):
    df = pd.read_csv('players_2020.csv')
    session = requests.Session()
    columns = ['id', 'type', 'timestamp', 'price']
    df_n = pd.DataFrame(columns=columns)
    df = df.iloc[core_id::4]
    ...
    for index, id in enumerate(ids):
        try:
            r = session.get(f'https://www.futwiz.com/en/app/price_history_player20?p={id}&c=xb').text
            r = r.replace('\n', '')
            j = json.loads(r)
            for p in j:
                df_n = pd.concat(
            ...
            pass
        try:
            r = session.get(f'https://www.futwiz.com/en/app/price_history_player20?p={id}&c=ps').text
            r = r.replace('\n', '')
            j = json.loads(r)
            for p in j:
                df_n = pd.concat(
...
def fifa_2020_history():
    ...
    n_multiprocessing = 4
    for i in range(n_multiprocessing):
        df_n.to_csv(f'players_2020_history_{i}.csv', index=False)
    pool = Pool(n_multiprocessing)
    # pass the function the id of the thread
